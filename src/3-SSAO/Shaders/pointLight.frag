...
uniform sampler2D g_buffer_ssao;

out vec3 out_color;

vec2 calculateScreenTexCoord() {
...
vec3 calculatePhong(vec3 normal, vec3 lightDirection, vec3 viewDirection, vec3 ambientColor, vec3 diffuseColor, vec3 specularColor) {
    ambientColor *= texture(g_buffer_ssao, calculateScreenTexCoord()).r;
...
void main() {
    ...
    vec2 texCoord = calculateScreenTexCoord();
